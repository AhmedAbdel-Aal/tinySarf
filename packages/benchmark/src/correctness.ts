...
export async function correctness(modelDirectory?:string,local=false,goldFiles:string[]=[],releaseCandidate=false,role:'verification'|'final-test'='verification') {
  ...
  const model=await loadModel(manifest,packed),split=await json(path.join(ROOT,"packages/training/data/split-manifest.json"));
  ...
  if(manifest.verificationDigest!==split.roles.verification.sha256 || hash(verificationBytes)!==split.roles[role].sha256 || hash(trainBytes)!==split.roles.train.sha256) throw new Error("Frozen dataset/checkpoint digest mismatch");
  ...
  const artifact={...provenance(),kind:"correctness",status:"experimental-unpromoted",model:manifest,data:{datasetId:split.datasetId,verificationDigest:manifest.verificationDigest,evaluationRole:role,evaluationDigest:hash(verificationBytes),goldDigest:goldFiles.length?goldDigests:null,trainDigest:hash(trainBytes),normalizationVersion:"arabic-v1"},
    protocol:{labelOrigin:"teacher-generated",distribution:"teacher-sampled lexical surfaces; not natural text",scoreSemantics:"ranking",threshold:null,bootstrap:{seed:42,resamples:2000,unit:"word"},finalTestOpened:role==='final-test'||goldFiles.length>0},
  ...
  const file=await writeArtifact(role==='final-test'?'final-test-correctness':"correctness",artifact,local?"packages/benchmark/local":"packages/benchmark/results");
...
if(import.meta.url===`file://${process.argv[1]}`) {
  const {values}=parseArgs({options:{"model-dir":{type:"string"},local:{type:"boolean",default:false},gold:{type:'string',multiple:true},'release-candidate':{type:'boolean',default:false},'dataset-role':{type:'string',default:'verification'}}});if(!['verification','final-test'].includes(values['dataset-role']!))throw new Error('Unknown evaluation role');await correctness(values['model-dir'],values.local,values.gold,values['release-candidate'],values['dataset-role'] as 'verification'|'final-test');
}
