...
export async function evaluateAI(file:string,local=false){
 ...
 const artifact={...provenance(),kind:'ai-review',status:'diagnostic-only-not-human-gold',model:manifest,data:{aiReviewDigest:hash(bytes),inputDigest:hash(inputBytes),verificationDigest:manifest.verificationDigest,goldDigest:null},
