...
def select(paths):
    ...
    model=Student(checkpoint['width'],checkpoint['labels']); model.load_state_dict(checkpoint['state']); model.eval()
    ...
    selection={'schemaVersion':1,'status':'experimental-unpromoted','id':path.name,'directory':str(destination.relative_to(ROOT)),
      'manifestSha256':sha(destination/'manifest.json'),'reason':'Lowest sampled verification cross-entropy among completed candidate sizes; no final-test access.',
