...
export interface ModelManifest {
  schemaVersion:1; format:"cnn-v1"; id:string; status:string; normalizationVersion:string;
...
export function validateManifest(m:ModelManifest,byteLength:number):void {
  if(m.schemaVersion!==1 || m.format!=="cnn-v1" || m.normalizationVersion!==NORMALIZATION_VERSION || m.quantization!=="symmetric-per-tensor-int8") throw new Error("Unsupported TinySarf model contract");
  ...
  for(const [name,classes] of Object.entries({segmentation:m.labels.segmentation,...m.labels.heads})) { shapes[`${name}.weight`]=[classes.length,m.width]; shapes[`${name}.bias`]=[classes.length]; }
  ...
  if(offset!==byteLength || count-m.embedding!==m.reachableWeights || count!==m.trainingParameters) throw new Error("Weight count mismatch");
}
