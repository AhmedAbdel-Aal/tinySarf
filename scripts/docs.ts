...
const sameModel=(candidate:any)=>JSON.stringify(candidate)===JSON.stringify(m);
...
if(values.refresh){
 ...
 for(const kind of ['correctness','size','browser','parity']) {
  for(const file of files.filter(f=>f.startsWith(kind+'-')&&f.endsWith('.json'))){const candidate=await json(path.join(results,file));if(sameModel(candidate.model)&&candidate.runtime?.sha256===runtimeSha256){artifacts[kind]={file:`packages/benchmark/results/${file}`,sha256:hash(await readFile(path.join(results,file)))};break;}}
  if(!artifacts[kind])throw new Error(`No measured ${kind} artifact for this checkpoint`);
 }
 const supplemental:any={};
 for(const kind of ['reproduction','ai-review','root-comparison'])for(const file of files.filter(f=>f.startsWith(kind+'-')&&f.endsWith('.json'))){const record=await json(path.join(results,file));if(sameModel(record.model)&&record.runtime?.sha256===runtimeSha256&&(kind!=='reproduction'||record.passed)){supplemental[kind]={file:`packages/benchmark/results/${file}`,sha256:hash(await readFile(path.join(results,file)))};break;}}
 ...
 const promotions=path.join(ROOT,'packages/training/promotions');for(const file of (await readdir(promotions)).filter(f=>/^\d.*Z\.json$/.test(f)).sort().reverse()){const record=await json(path.join(promotions,file));if(!sameModel(record.model))continue;supplemental.eligibility={file:`packages/training/promotions/${file}`,sha256:hash(await readFile(path.join(promotions,file)))};break;}
...
const rootConfig=config.rootTraining;
...
- Experimental package version: ${(await json(path.join(ROOT,'packages/core/package.json'))).version}. Distributed through [GitHub Releases](https://github.com/AhmedAbdel-Aal/tinySarf/releases); no npm registry release. See the [README](README.md) for the direct installation command and public browser demo.
- Promoted checkpoint and timestamp: **none**. Experimental candidate: \`${m.id}\`.
- Architecture: ${m.format}; multi-task embedding width ${m.embedding}, convolution width ${m.width}, dilations ${m.dilations.join('/')}.${m.rootArchitecture?` A separate root CNN uses ${m.rootArchitecture.embedding}-dimensional embeddings, width ${m.rootArchitecture.width}, and character copying mixed with learned radical restoration. It replaces the four pooled root classifiers.`:' Three full convolutions; no affine scan in this experiment.'}
- Training parameters: ${num(m.trainingParameters)}. Reachable deployed weights: ${num(m.reachableWeights)}. ${m.rootArchitecture?'Both zero padding embedding rows are':'The zero padding embedding row is'} excluded from the reachable count.
...
${rootConfig?`The multi-task encoder and every non-root tensor are preserved from the original int8 checkpoint. The root CNN is trained from scratch. Its loss marginalizes over unique complete allowed roots, so repeated teacher analyses do not multiply a root's weight and incompatible radical combinations are not treated as labels. Four attention heads can copy characters; learned generators restore radicals absent from the written form. AdamW uses learning rate ${rootConfig.learningRate??.003}, weight decay ${rootConfig.weightDecay??.001}, batch ${rootConfig.batchSize??128}, seed ${rootConfig.seed}, and gradient clipping at ${rootConfig.gradientClip??5}. Root decoding also uses ${num(m.rootDecoder.whole.length)} whole-word and ${num(m.rootDecoder.stem.length)} stem transformation rules induced from training, each supported by at least ${m.rootDecoder.minimumDistinctRoots} different roots. The package contains no word-to-root dictionary.`:`Character and positional embeddings feed three masked ReLU convolutions, masked mean pooling, per-character segmentation and fixed classification heads. Uniformly sample an accepted mappable analysis per word each epoch. Segmentation cross-entropy receives weight 2; the loss averages this and all head losses. AdamW, learning rate ${config.learningRate}, weight decay ${config.weightDecay}, batch ${config.batchSize}, seed ${config.seed}, gradient clipping at 1. Deterministic CPU execution with ${config.threads} threads. No curriculum or replay was used in this candidate.`}
...
${rootConfig?`The root training run evaluated ${rootConfig.epochs} epochs and selected epoch ${training.training.bestEpoch} by lowest verification marginal root loss. Transformation-score settings use teacher verification root agreement. The root loss is not comparable to the older multi-task losses in the table. All experiments and later, worse epochs are retained. No final-test labels or AI annotation labels were used for training. AI challenge results are diagnostic rather than an untouched model-selection guarantee.`:`The selected candidate continued the initial 250K run for ${config.epochs} additional epochs and selected continuation epoch ${training.training.bestEpoch} by lowest deterministic sampled verification loss. Optimizer state resets for continuation. All later, worse epochs remain in the saved history. Selection never used the sealed final test. This small data sample has not established useful root or full-analysis quality.`}

Environment: PyTorch ${rootConfig?.torch??config.torch??'2.5.1'}, NumPy ${rootConfig?.numpy??config.numpy??'1.26.4'}; training device ${rootConfig?.device??config.device??'cpu'}; initial machine ${b.environment.cpu}, ${num(b.environment.ramBytes)} bytes RAM. Pinned Python dependencies are in [requirements.lock](packages/training/requirements.lock). The separate CPU/NumPy reference defines exported semantics; int8 uses independent symmetric per-tensor rounding.
...
The [annotation audit](packages/training/data/ai-review/README.md) preserves source labels, disagreements, adjudication decisions and input hashes. These labels were not used as training targets. ${rootConfig?'The challenge was inspected during root development and is therefore a development-exposed diagnostic, not an untouched holdout. Root training excludes its normalized words and teacher lemma families; the inherited multi-task encoder retains the legacy overlap shown above.':'The current model was not selected using these annotations.'}
...
const rootComparison=supplemental['root-comparison'];
if(rootComparison){
 const section=`### Root redesign, paired comparison

The same ${q.count} verification words and exact teacher root strings are used for both models. Segmentation, POS, pattern and feature top-1 changes: ${JSON.stringify(rootComparison.changedNonRootTop1)}. This is repeated development data, not independent human accuracy. ${evidenceLink('root-comparison')}.

${table(['Slice','Original root agreement','Current root agreement','Change / 95% CI, percentage points'],Object.entries(rootComparison.slices).map(([name,v]:[string,any])=>[name,rate(v.baseline),rate(v.candidate),v.delta.difference==null?'TBD':`${(v.delta.difference*100).toFixed(2)} (${(v.delta.ci95.low*100).toFixed(2)} to ${(v.delta.ci95.high*100).toFixed(2)})`]))}

Fixed errors: ${rootComparison.outcomes.fixed}; regressions: ${rootComparison.outcomes.regressed}. Relative root-error reduction: ${pct(rootComparison.relativeRootErrorReduction)}. The unseen-root comparison above keeps the original training-root definition fixed for both models. The ordinary correctness slices instead use all current root training pools. [Experiment design, ablations and commands](docs/ROOT_EXPERIMENT.md).

`;
 text=text.replace('### Gold accuracy',section+'### Gold accuracy');
}
if(corrected){
...
const website=path.join(ROOT,'apps/website');await writeFile(path.join(website,'lib/results.json'),JSON.stringify({entries:[{label:'Reachable weights',value:num(m.reachableWeights),note:`Int8 · ${num(m.packedWeightsBytes)} packed bytes`},{label:'Teacher segmentation agreement',value:pct(q.segmentationExactMatch.value),note:`${num(q.count)} teacher-sampled verification words`},{label:'Teacher root agreement',value:pct(q.rootExactMatch.value),note:'Exact teacher root strings; independent human gold is absent'},{label:'WebGPU batch advantage',value:batch?`${batch.speedup.toFixed(2)}×`:'Not measured',note:`Batch 128 vs JavaScript reference · ${b.protocol.mode}`}],note:'Experimental, unpromoted. Full-analysis and POS quality remain below release targets; independent human gold is absent. One headless Chromium / Apple M2 measurement; not a cross-browser claim.'+(ai?` An additional ${ai.counts.inputs}-input challenge was independently annotated by two AI agents and adjudicated by a third; see the model card for AI agreement and its limitations.`:''),artifact:'/results/current.json'},null,2)+'\n');
